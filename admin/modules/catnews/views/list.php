  ...
  <!--DWLayoutTable-->	
	 <tr valign="center">  	
	   <th height="20" align="center" valign="middle"><?php echo ALD ? '<input name="allbox" type="checkbox" id="allbox" onclick="CheckAll(this)">' : '&nbsp;';?></th>
			...
			<th align="left" valign="middle" nowrap="nowrap" title="<?php echo $this->descriptionLangList[$i];?>" >Category Name (<font color="#FF0000"><?php echo strtoupper($this->keyLangList[$i]);?></font>)</th>
			<?php endfor;?>
			<th  align="center" valign="middle" nowrap="nowrap" >News Items Count </th>
		...
		<?php if($row->display):?>
		<tr style="<?php echo $row->isgallery ? 'font-weight:bold;' : '';?>">
		<?php else:?>
		<tr style="background:#cccccc; <?php echo $row->isgallery ? 'font-weight:bold;' : '';?>">
		<?php endif;?>
		 <td align="center" class="content-rows"><?php echo ALD ? "<input type=\"checkbox\" name=\"delete[]\" value=\"{$row->id}\">" : '&nbsp;';?></td>
			...
			<?php endfor;?>
			<td align="center"  valign="middle"  class="content-rows">[<span class="d"><?php echo $this->PRO_GetCountNews($row->id);?></span>] - <a href="index.php?module=<?php echo MODULE_CHILD_NAME;?>&menu=<?php echo $row->menu_id?>&cat=<?php echo $row->id?>">detail</a></td><?php if (ALSD):?><td align="center"  valign="middle"  class="content-rows"><?php echo $this->__auto_write_checkbox('display', $row->id, $row->display)?></td><?php endif;?>
