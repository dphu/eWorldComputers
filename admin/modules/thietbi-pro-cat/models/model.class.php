...
class PRO_CAT_MODEL
{
	...
	public static function autoCreateTables(&$db)
	{
		...
		PRO_CAT_MODEL::PRI_UpdateTitleKeywordDescription($db);
	}
	
	private static function PRI_UpdateTitleKeywordDescription(&$db)
	{
		__MYMODEL::__doUPDATE($db, PRO_CAT_TABLE_NAME, array('`pagetitle` = `name_en`'), "WHERE (`pagetitle` = '') OR (`pagetitle` IS NULL)");
		__MYMODEL::__doUPDATE($db, PRO_CAT_TABLE_NAME, array('`metakeyword` = `name_en`'), "WHERE (`metakeyword` = '') OR (`metakeyword` IS NULL)");
		__MYMODEL::__doUPDATE($db, PRO_CAT_TABLE_NAME, array('`metadescription` = `name_en`'), "WHERE (`metadescription` = '') OR (`metadescription` IS NULL)");
	}
