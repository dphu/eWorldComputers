...
class MODEL
{
  ...
  PRIMARY KEY  (`id`)
		...
		MODEL::PRI_UpdateTitleKeywordDescription($db);
	}
	
	private static function PRI_UpdateTitleKeywordDescription(&$db)
	{
		__MYMODEL::__doUPDATE($db, TABLE_NAME, array('`pagetitle` = `name_en`'), "WHERE (`pagetitle` = '') OR (`pagetitle` IS NULL)");
		__MYMODEL::__doUPDATE($db, TABLE_NAME, array('`metakeyword` = `name_en`'), "WHERE (`metakeyword` = '') OR (`metakeyword` IS NULL)");
		__MYMODEL::__doUPDATE($db, TABLE_NAME, array('`metadescription` = `name_en`'), "WHERE (`metadescription` = '') OR (`metadescription` IS NULL)");
	}
