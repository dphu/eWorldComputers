...
class HTTP_Download {
        ...
        foreach ((array) $params as $param => $value) {
            ...
            if (!method_exists($this, $method)) {
                return PEAR::raiseError(
                                "Method '$method' doesn't exist.", HTTP_DOWNLOAD_E_INVALID_PARAM
                );
            }
    ...
    function setFile($file, $send_404 = true) {
        ...
        if (!is_file($file)) {
            ...
            return PEAR::raiseError(
                            "File '$file' not found.", HTTP_DOWNLOAD_E_INVALID_FILE
            );
        }
    ...
    function setGzip($gzip = false) {
        if ($gzip && !PEAR::loadExtension('zlib')) {
            return PEAR::raiseError(
                            'GZIP compression (ext/zlib) not available.', HTTP_DOWNLOAD_E_NO_EXT_ZLIB
            );
        }
        $this->gzip = (bool) $gzip;
        return true;
    ...
    function setBufferSize($bytes = 2097152) {
        if (0 >= $bytes) {
            return PEAR::raiseError(
                            'Buffer size must be greater than 0 bytes (' . $bytes . ' given)', HTTP_DOWNLOAD_E_INVALID_PARAM);
        }
    ...
    function setContentType($content_type = 'application/x-octetstream') {
        if (!preg_match('/^[a-z]+\w*\/[a-z]+[\w.;= -]*$/', $content_type)) {
            return PEAR::raiseError(
                            "Invalid content type '$content_type' supplied.", HTTP_DOWNLOAD_E_INVALID_CONTENT_TYPE
            );
        }
    ...
    function guessContentType() {
        ...
        if (!function_exists('mime_content_type')) {
            return PEAR::raiseError(
                            'This feature requires ext/mime_magic!', HTTP_DOWNLOAD_E_NO_EXT_MMAGIC
            );
        }
        if (!is_file(ini_get('mime_magic.magicfile'))) {
            return PEAR::raiseError(
                            'ext/mime_magic is loaded but not properly configured!', HTTP_DOWNLOAD_E_NO_EXT_MMAGIC
            );
        }
        if (!$content_type = @mime_content_type($this->file)) {
            return PEAR::raiseError(
                            'Couldn\'t guess content type with mime_content_type().', HTTP_DOWNLOAD_E_INVALID_CONTENT_TYPE
            );
        }
    ...
    function send($autoSetContentDisposition = true) {
        if (headers_sent()) {
            return PEAR::raiseError(
                            'Headers already sent.', HTTP_DOWNLOAD_E_HEADERS_SENT
            );
        }
    ...
    function sendArchive($name, $files, $type = HTTP_DOWNLOAD_TGZ, $add_path = '', $strip_path = '') {
        ...
        return HTTP_Download_Archive::send($name, $files, $type, $add_path, $strip_path);
    }
    ...
    function sendChunk($chunk, $cType = null, $bound = null) {
        ...
        if ($length < 1) {
            return PEAR::raiseError(
                            "Error processing range request: $offset-$lastbyte/$length", HTTP_DOWNLOAD_E_INVALID_REQUEST
            );
        }
